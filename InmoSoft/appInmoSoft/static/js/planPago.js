...
const cuotaInicialInput = document.getElementById("txtCuotaIni");
...
document.getElementById("btnCalcularPlan").addEventListener("click", function() {
    const cuotaInicial = parseFloat(cuotaInicialInput.value);
    ...
    var treintaPorCiento = valorInmueble * 0.3;
    // Calcula el valor de cada cuota mensual
    const valorPorCuota = (treintaPorCiento - (cuotaInicial+valorSeparacion)) / numCuotas;
    ...
    for (let i = 0; i < numCuotas; i++) {
        ...
        planPagoCell.innerHTML = `$${(cuotaInicial).toFixed(2)}`;
        mesesCell.innerHTML = fecha.toLocaleString('default', { month: 'long' });
        cuotaCell.innerHTML = `$${valorPorCuota.toFixed(2)}`;
    }
});
