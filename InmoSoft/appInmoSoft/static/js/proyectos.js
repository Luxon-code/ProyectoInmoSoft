...
function fotos(selctId,fileId,ulId){
      /*
  * Variables
  */
  ...
  const fileInputMulti = document.querySelector(`#${selctId} #${fileId}`);
  // DEMO Preview
  const multiSelectorUniqPreview = document.querySelector(`#${selctId} #${ulId}`);
  ...
  */
  ...
  function renderPreviews(currentFileList, target, inputFile) {
      ...
      currentFileList.forEach((file, index) => {
          ...
          myButtonRemove.className = 'btn text-danger';
